...
class Simplex {
    vector<double> c; // значения целевой функции
    ...
    vector<vector<double>> table; // таблица

    // начальные условия
    vector<vector<double>> initialA;
    vector<double> initialC;
    vector<double> initialB;

    int padding; // отступ

    void PrintVector(const vector<double> &v) const;
    void PrintHeader() const;
    void PrintLine() const;
    void PrintSolve(SimplexSolve solve) const;
    ...
    void DivideRow(int row, double value); // деление строки на число
    void SubstractRow(int row1, int row2, double value); // вычитание строки row2 * value из row1
    void Gauss(int row, int column); // исключение гаусса

    bool IsOptimal(); // проверка плана на оптимальность
    bool CheckSolve(const vector<double> x) const; // подходит ли решение по условию
    void CalculateDeltas(); // расчёт дельт
    vector<double> CalculateSimplexRelations(int columnIindex); // расчёт симплекс отношений

    int GetSolveColumn(); // получение разрешающего столбца
    int GetSolveRow(const vector<double> &q); // получение разрешающей строки
    SimplexSolve GetSolve(); // получение решения

    int GetRealIndex(const vector<double> &x); // получение индекса вещественного решения
public:
    Simplex(const vector<vector<double>> &a, const vector<double> &b, const vector<double> &c, SimplexMode mode, int padding = 0);

    void PrintTable() const; // вывод таблицы
    bool Solve(bool debug = true); // решение задачи

    vector<SimplexSolve> SolveIntegerBranchesAndBorders(bool debug = true, int depth = 0); // получение целочисленных решений
    vector<SimplexSolve> SolveIntegerBruteforce(int nmax); // поиск полным перебором

    void FindBestSolve(const vector<SimplexSolve> &solves) const; // поиск лучшего из решений
};

Simplex::Simplex(const vector<vector<double>> &a, const vector<double> &b, const vector<double> &c, SimplexMode mode, int padding) {
    this->mode = mode;
    ...
    for (int i = 0; i < m; i++) {
        this->table.push_back(vector<double>(n + m + 1, 0)); // добавляем строку в таблицу
    ...
    initialA = vector<vector<double>>(a);
    initialB = vector<double>(b);
    initialC = vector<double>(c);
}

void Simplex::PrintVector(const vector<double> &v) const {
...
void Simplex::DivideRow(int row, double value) {
    for (int i = 0; i < n + m + 1; i++)
        table[row][i] /= value;
}

// вычитание строки row2 * value из row1
void Simplex::SubstractRow(int row1, int row2, double value) {
    for (int i = 0; i < n + m + 1; i++)
...
bool Simplex::CheckSolve(const vector<double> x) const {
    for (int i = 0; i < m; i++) {
        double sum = 0;
...
vector<double> Simplex::CalculateSimplexRelations(int columnIndex) {
    vector<double> q;

    for (int i = 0; i < m; i++) {
        if (fabs(table[i][columnIndex]) < EPS) {
            q.push_back(INF);
        }
        else {
...
int Simplex::GetSolveRow(const vector<double> &q) {
    int row = -1;
...
bool Simplex::Solve(bool debug) {
    ...
    for (int iteration = 1; true; iteration++) {
        ...
        vector<double> q = CalculateSimplexRelations(column); // рассчитываем симлекс-отношения
...
int Simplex::GetRealIndex(const vector<double> &x) {
    for (int i = 0; i < n; i++)
        if (fabs(x[i] - int(x[i])) > EPS) // если не совпадает с целой частью
...
vector<SimplexSolve> Simplex::SolveIntegerBranchesAndBorders(bool debug, int depth) {
    ...
    double b = int(solve.x[realIndex]); // получаем новое условие
    
    // готовим новые условия
    vector<vector<double>> a1 = initialA;
    vector<vector<double>> a2 = initialA;

    vector<double> b1 = initialB;
    vector<double> b2 = initialB;

    a1.push_back(vector<double>(n, 0));
    a1[m][realIndex] = 1;
    b1.push_back(b);

    a2.push_back(vector<double>(n, 0));
    a2[m][realIndex] = -1;
    b2.push_back(-b - 1);
...
// поиск решений полным перебором
vector<SimplexSolve> Simplex::SolveIntegerBruteforce(int nmax) {
    vector<SimplexSolve> solves;
    cout << "Bruteforce solves: " << endl;

    for (double x1 = 0; x1 <= nmax; x1++) {
        for (double x2 = 0; x2 <= nmax; x2++) {
            for (double x3 = 0; x3 <= nmax; x3++) {
                SimplexSolve solve;
                solve.x = { x1, x2, x3 };
